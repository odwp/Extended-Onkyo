...
metadata 
{
	definition (name: "Onkyo Multi-Zone AVR Parent - Extended", namespace: "SteveV", author: "Steve Vibert")
	{
...
def findZones()
{
    log.warn "Looking for Zone: Main." 
    sendTelnetMsg(getEiscpMessage("PWRQSTN"))
    log.warn "Looking for Zone: Zone2." 
    sendTelnetMsg(getEiscpMessage("ZPWQSTN"))
    log.warn "Looking for Zone: Zone3." 
    sendTelnetMsg(getEiscpMessage("PW3QSTN"))
    log.warn "Looking for Zone: Zone4." 
    ...
    try 
    {
        ...
        enabledReceiverZones.each { it ->
            ...
            if(child == null) 
            {
                ...
                def childType = "Onkyo Multi-Zone AVR Child Zone - Extended 2"
